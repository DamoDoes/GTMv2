"""Tax Filing Agent — research assistant for tax filing patterns and EITC opportunities."""
...
from agents.tax_filing.prompts import SYSTEM_PROMPT
from agents.tax_filing.tools import TOOLS
...
class TaxFilingAgent:
    """Agentic loop for tax filing research queries."""
    ...
    def run(self, query: str, *, verbose: bool = False) -> str:
        ...
        for _ in range(10):
            ...
            if verbose:
                print(f"[tax-filing] stop_reason={response.stop_reason}")
            ...
            for tool_use in tool_uses:
                if verbose:
                    print(f"[tax-filing] calling {tool_use.name}({tool_use.input})")
                result_str = _execute_tool(tool_use.name, tool_use.input)
                if verbose:
                    print(f"[tax-filing] -> {result_str[:200]}")
                tool_results.append({
                    "type": "tool_result",
